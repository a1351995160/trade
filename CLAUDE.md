...
### TQ窗口与输入类型实证（2026-09-11）
- 本机get_trading_calendar返回Date；不要把未知响应结构默认为空日历。get_divid_factors曾忽略TRAIN日期返回跨窗事件，当前新适配必须拒绝再次调用，不能先全收后过滤。
- 空TDX片段与pandas 3合并可能令价格列为object；规范化时显式校验数值类型，保留缺失值。每个输入阶段先持久审计，再执行后续因子计算，避免异常丢失已发生读取的证据。

### 固定账户公司行动接线（2026-09-11）
- 原 OrderManager.submit 已完成受理，不存在额外 accept 方法。公司行动替换待成交退出订单时必须保留原 eligible_at；用事件当天新建订单而丢失它会把本应当天成交的卖单错延一个 session。
- 原历史状态 available_at 可能来自下一 session 开盘模型，不能当历史发布证据。新 observed 时间需来源和原生产者语义证明，单独保留 effective 字段；更晚真实时间优先。混合 ISO 时间格式应显式 ISO8601 并验证时区。
- 数百万状态行不可整体转为 Python 字典副本；使用索引表按证券日查询。合成通过不代表真实规模资源通过；原限制器仍须实际裁决。

### OWNER导出实证（2026-09-11）
- 本机17709已监听而TQ请求502时，先查系统代理是否把127.0.0.1发往代理；本用途仅在OWNER进程设置NO_PROXY，不修改公共客户端、鉴权或扩大网络访问。
- gbbq包含非沪深市场不代表解码失败，先验证记录格式，再按获准证券身份筛选；全历史只留在OWNER进程内，源/计数审计必须先于可能失败的字段校验。
- 两份TQ文档可能有单位冲突；价格逐行一致不代表成交量编码也通过。保留原冻结容差失败，不以量差很小、多数一致或浮点猜测自动认定兼容。
- 对混合JSON状态文件只读头部时必须禁用缓冲预取并在rows前停止；不能将gbbq专属的OWNER全历史读取例外推广到history文件。
- Windows的Path.read_text默认可能为GBK；OWNER导出合同是UTF-8，读取请求、单位、事件manifest和中文报告都必须显式指定编码，测试应包含未转义中文。

### 降级账户日历与数量实证（2026-09-11）
- 原Ledger自然日+1可生成周末sellable_from，不能直接假定它属于独立交易日历。降级接线须只向后投影至真实session并保留old/new审计；合成回归必须包含周五入场。真实失败已消费的额度不因red/green通过而退回。
- DAY数量量纲证明与跨提供者数值相等是两个不同命题；SHARES推导通过不覆盖旧1-share失败，也不升级为厂商版本背书。修复读取日志如沿用MAIN标签，保留原件并用权威reservation与补充访问记录明确实际REPAIR用途。

### BaoStock价格模式实证（2026-09-12）
- BaoStock的1是后复权、2是前复权、3是不复权。测试夹具必须对照提供者定义，不能复制实现中的错误标签后宣称接通。
- 后复权响应曾在上市最初日期返回flag3。只有独立上市状态、官方初始因子1、RAW前收递推和同日数值恒等均证明时，才可按降级输入版本接受初始前缀；保留原标志和失败，不能全局放行flag3或声称厂商已解释标签。
