...
## 2026-09-12 - Task: 核对BaoStock超时恢复条件并形成具体决定
### What was done
- 只读定位batch6整体超时与002853.SZ后复权中断请求；确认响应/访问回执不存在、同入口进程不存在。
- 对账资源累计3550.1620778秒，原余额1849.8379222秒。提出保留缓存、缩小调度批次、单对象一次补取及累计360分钟的待批调整，不改旧合同或额度。
### Testing
- 实际读取批次资源、精确中断对象元数据及Windows进程查询，未读行情正文、未请求网络；以各成功批次耗时估算剩余工期。
- 文档与已批准包的一次请求、90分钟及900秒边界交叉核对；没有新增代码，不运行研究测试或实验。
### Notes
...
## 2026-09-12 - Task: 定位第15批真实质量失败与重复证券身份
### What was done
- 精确核对batch14，仅302132.SZ的492行HFQ模式失败，未放宽校验、重试接口或覆盖原件。
- 核对既有300114.SZ与302132.SZ原始响应除code外全部字段相同；发行人公开实施公告说明代码变更生效日2025-02-17，在TRAIN之后。
- 保存定点诊断证据，提出单对证券身份规范化的具体决定。现有冻结要求5182成员，执行身份规范化须明确确认，当前未擅自删成员或恢复取数。
### Testing
- 四份原始响应逐一匹配原访问回执SHA256；两代码492行RAW日期/OHLC/前收/量额/状态全部一致。
- 新代码HFQ全部返回flag3且数值等于RAW，2023-06-06前收断点否定直接套用已有恒等规则；旧代码HFQ全部flag1。
- 没有修改生产代码、没有绩效试验或额外网络行情请求；git diff --check通过。
### Notes
- docs/baostock-batch14-identity-resolution-v1.md：具体归因、冻结身份影响及建议修复。
- progress.md：追加记录。
