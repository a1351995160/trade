...
class DegradedGovernanceV1(TrainExecutionGovernanceV1):
    ...
    def confirm(self, plan, source, *, preflight):
        ...
        if (len(plan['contracts'])!=1 or plan['limit']!=2 or plan['wall_limit']!=1800
                or plan['result_type']!=CONTRACT['result_type'] or list(plan['contracts'].values())!=[CONTRACT]):
            raise ValueError('DEGRADED_FIXED_SCOPE_MISMATCH')
        expiry=datetime.fromisoformat(plan['expires_at'])
        if not datetime.now(timezone.utc)<expiry<=datetime.fromisoformat(CONTRACT['expires_at']):
        ...
        with self.lock():
            ...
            receipt={'schema_version':'degraded-train-confirmation-v1','plan':plan,'source':source,
                ...
                'main_purpose':'DEGRADED_TRAIN_ACCOUNT_MAIN','repair_purpose':'DEGRADED_TRAIN_ACCOUNT_REPAIR'}
