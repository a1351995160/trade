...
from chanlun_trader.research_factory.formal_statistics_v2 import (
...
def run(output, workers, *, preregister_only=False):
    ...
    with ObjectiveMutationLock.for_resource(output / 'calibration'):
        ...
        if prereg_path.exists():
            ...
            if (prereg['spec'] != CALIBRATION_SPEC or prereg['sources'] != source_hashes()
                    or prereg['method_hash'] != METHOD_HASH):
        ...
        if preregister_only:
            print({'status':'PREREGISTERED_ONLY','method_hash':METHOD_HASH,'output':str(output)}, flush=True)
            return
        ...
        report = {'schema_version': 'FORMAL_STATISTICS_CALIBRATION_REPORT_V2',
